...
from .deps.command_deps import fs_ref_from_command, fs_snap_exists
...
async def create_command(
    cmd: Command, ref: AsyncDocumentReference = Depends(fs_ref_from_command)
):
    try:
        await ref.create(cmd.to_snap())
    except Conflict:
        raise HTTPException(
    ...
    return Command.from_snap(await ref.get())
...
async def update_command(
    ...
    await snap.reference.update(updated_command.to_snap())
